...
const int sampleRate = 5000; //500 à 44100, plus elle est basse, plus les fichiers seront légers
const int bitDepth = 16;
const float t0 = 0.5; //durée d'un point
int f = 1000; //D'après le théorème de Niquyst-Shannon, il faut sampleRate>2*f
...
public:
    ...
    void writeTextFile(char* path, std::string data) {
        ...
        while(!(newfile)) {
            std::cout << "mauvais nom de fichier, réessayez" << std::endl;
            std::cin >> path;
...
public:
    ...
    std::vector<int16_t> readWav(char* path, std::vector<float>& Lt){
        ...
        while(wavFile==NULL) {
            std::cout<< "mauvais chemin, réessayez" << std::endl;
            std::cin >> path;
...
public:
    ...
    void Alpha_Morse() {
        ...
        char path1[100];
        std::string chaine;
        while(!(mode_Entree=="Y")||(mode_Entree=="N")) {
            std::cout << "mauvaise commande, rééssayez" << std::endl;
            std::getline(std::cin,mode_Entree);
        }
        if (mode_Entree=="Y") {
        ...
        w.MorseIntoWav(t.alphaIntoMorse(chaine),path2);
    ...
    void Morse_Alpha() {
        ...
        char path1[300];
        std::cin.get(path1,300);

        std::vector <float> Lt;
        std::string sortie = t.morseIntoAlpha(r.WavIntoMorse(Lt,r.readWav(path1,Lt)));
        ...
        if (choice=="Y") {
            ...
            w.writeTextFile(path2,sortie);
            std::cout << "fichier créé au chemin : " << path2 << std::endl;;
        }
